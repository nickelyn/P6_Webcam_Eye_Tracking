...
def getFace(frame):
    ...
    # ref_image = cv.imread("src/distance_detection/Ref_image_new.jpg")
    ...
    for face in faces:
        ...
        re_horizontal_line = cv.line(
            frame, re_left_point, re_right_point, (0, 255, 0), 1
        )
        re_vertical_line = cv.line(
            frame, re_center_top_point, re_center_bottom_point, (0, 255, 0), 1
        )

        # Find the length of the vertical line for the right and left eye
        re_vertical_line_len = getVerticalLen(
            re_center_top_point[0],
            re_center_top_point[1],
            re_center_bottom_point[0],
            re_center_bottom_point[1],
        )
        le_vertical_line_len = getVerticalLen(
            le_center_top_point[0],
            le_center_bottom_point[0],
            le_center_top_point[1],
            le_center_bottom_point[1],
        )

        # Find the length of the horizontal line for right and left eye
        re_horizontal_line_len = getVerticalLen(
            re_left_point[0], re_right_point[0], re_left_point[1], re_right_point[1]
        )
        le_horizontal_line_len = getVerticalLen(
            le_left_point[0], le_right_point[0], le_left_point[1], le_right_point[1]
        )
