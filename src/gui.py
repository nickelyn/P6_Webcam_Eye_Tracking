...
class Gui:
    ...
    camera_feed = [
        [sg.Image(filename="", key="frame", size=(200, 200))]
    ]

    window_feed = [
        [sg.Image(filename="", key="window", size=(200, 200))]
    ]

    toggle = [
        [sg.Text("Turn webcam On/Off")],
        [sg.Button('Off', size=(5,1), button_color=('white', 'red'), key='_TOGGLE_')]
    ]
    ...
    select_window = [
        ...
        [sg.Combo(WINDOW_LIST, enable_events=True, readonly=True, font="Consolas 10", key='SELECT')],
        [sg.Button("Apply")]
    ]
    ...
    def __init__(self) -> None:
        self.layout = [
            [sg.Column(self.left_col), sg.VSeperator(), sg.Column(self.right_col)]             
        ]

        self.window = sg.Window("{} {}".format(APP_NAME, APP_VERSION), 
                                self.layout, default_button_element_size=(12, 1),
                                auto_size_buttons=False, resizable=True, 
                                element_justification='center', font='Helvetica 16')